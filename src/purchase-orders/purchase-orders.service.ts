  ...
  async update(id: string, dto: UpdatePurchaseOrderDto) {
    try {
      return this.databaseService.$transaction(async (prisma) => {
        ...
        if (items.length === 0) {
          throw new BadRequestException('At least one item must be provided for update');
        }
        ...
        await Promise.all(
          existingOrder.items.map(async (orderItem) => {
            const prItem = purchaseRequest.items.find(i => i.itemId === orderItem.itemId);
            if (!prItem) return;
        ...
        const validItems = await Promise.all(
          items.map(async (newItem) => {
            const prItem = purchaseRequest.items.find(i => i.itemId === newItem.itemId);
            
            if (!prItem) {
              errors.push(`Item ${newItem.itemId} not found in original request`);
              return null;
            }
        ...
        // Remove null items
        const filteredValidItems = validItems.filter((item) => item !== null) as {
          itemId: string;
          quantity: number;
          price: number;
          remainingQty: number;
        ...
        const totalQty = filteredValidItems.reduce((sum, item) => sum + item.quantity, 0);
